...
import { createCashout, updateCashout, getCashoutById, getCashoutHistoryByEmployee, getCashoutByTxHash, getCashoutByPayoutId, getLifetimeTotalVolumeByBankAccount } from '../services/cashoutService';
 ...
 */
const SKIP_CASHOUT_SIGNATURE = process.env.SKIP_CASHOUT_SIGNATURE === '1';
...
export async function requestCashout(req: Request, res: Response) {
  try {
    ...
    // Optional signature verification (disabled on Tempo testnet)
    if (!SKIP_CASHOUT_SIGNATURE) {
    ...
    // Đợi transaction được confirm (Tempo finality nhanh nhưng đợi cho chắc)
    try {
      const { waitForTransaction } = await import('../utils/blockchain');
      await waitForTransaction(txHash);
    } catch (error: any) {
      console.warn('[Cashout] Failed to wait for transaction confirmation, continuing anyway:', error.message);
      // Continue anyway - có thể tx đã confirm rồi
    }
    ...
    try {
      ...
      // Nếu transaction không tìm thấy, có thể chưa được confirm - retry một lần
      if (error.message.includes('Transaction not found') || error.message.includes('not found')) {
        console.log('[Cashout] Transaction not found, waiting 2s and retrying...');
        ...
        try {
          ...
          return res.status(400).json({ 
            ...
            txHash,
            suggestion: 'Please wait a few seconds and try again, or check the transaction hash on Tempo explorer',
          });
        }
      } else {
        return res.status(400).json({ 
    ...
    // Với Tempo AA (Account Abstraction), tx được gửi từ smart account address,
    // không phải từ Passkey address trực tiếp. Ta chỉ cần verify:
    // 1. `to` đúng là offramp address
    // 2. `amount` đúng với amount user request
    // Note: `from` sẽ là smart account address (có thể khác với employeeAddress Passkey)

    if (transferInfo.to.toLowerCase() !== offrampAddress.toLowerCase()) {
      return res.status(400).json({ 
        error: 'Transaction recipient is not the offramp wallet',
        expected: offrampAddress,
        received: transferInfo.to,
      });
    }
    ...
    // Log thông tin để debug (không fail nếu không match)
    if (transferInfo.from.toLowerCase() !== employeeAddress.toLowerCase()) {
      console.log(
        `[Cashout] Transfer from smart account ${transferInfo.from} (expected Passkey address ${employeeAddress})`,
      );
      console.log(`[Cashout] TX from: ${transferInfo.txFrom || 'N/A'}`);
    }
 ...
 * Get lifetime total volume (total fiat amount) for a bank account
 */
export async function getLifetimeVolume(req: Request, res: Response) {
  try {
    ...
    // Decode bankAccountId (có thể bị encode từ frontend)
    const decodedBankAccountId = decodeURIComponent(bankAccountId);
    console.log(`[Cashout] Getting lifetime volume for bankAccountId: ${decodedBankAccountId}`);

    const result = await getLifetimeTotalVolumeByBankAccount(decodedBankAccountId);

    console.log(`[Cashout] Lifetime volume result:`, result);
    ...
    console.error('[Cashout] Error getting lifetime volume:', error);
    res.status(500).json({ error: error.message || 'Failed to get lifetime volume' });
  }
}
